<script setup>
  ...
  watch(selectedSeason, val => {
    router.push(`/admin/${selectedLeague.value}/${val}`);
  });
  ...
  const saveNewSeason = async () => {
    ...
    try {
      let { error, data: newSeason } = await supabase
        .from('season')
        .insert({ name: seasonName.value, league: selectedLeague.value })
        .single();
      if (error) {
  ...
  onMounted(async () => {
    ...
    try {
      ...
      if (error) {
        ...
        if (route.params.season && !seasons.value.find(season => season.id === parseInt(route.params.season))) {
...
<template>
  <div class="bg-white shadow rounded-lg divide-y divide-gray-200">
    ...
    <div v-if="loadError || (seasons.length > 0 && selectedSeason)" class="px-4 py-5 sm:p-1">
      ...
      <div v-if="selectedSeason" class="px-2 py-2.5 sm:p-3">
        <router-view />
      </div>
    </div>
  </div>
